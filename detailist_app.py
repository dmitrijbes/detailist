...
class DetailistApp:
    def __init__(self):
        ...
        self.screenshot_size = (self.screen_one_third_h, self.screen_one_third_h)
            ...
            [
                gui.Column(
                    [
                        [
                            gui.Button(
                                image_filename=self.icons_path + "save_icon.png",
                                button_color=button_color,
                                button_type=gui.BUTTON_TYPE_SAVEAS_FILE,
                            ...
                            gui.Button(
                                image_filename=self.icons_path + "center_icon.png",
                                button_color=button_color,
                                tooltip="Center",
                                key="center_graph_1",
                            ),
                            gui.Button(
                                image_filename=self.icons_path
                                + "center_as_right_icon.png",
                                button_color=button_color,
                                tooltip="Center As Right",
                                key="center_as_right",
                            ),
                            gui.Button(
                                image_filename=self.icons_path + "ocr_icon.png",
                                button_color=button_color,
                                tooltip="OCR",
                                key="ocr_graph_1",
                            ),
                            gui.Button(
                                image_filename=self.icons_path + "clear_icon.png",
                                button_color=button_color,
                                tooltip="Clear",
                                key="clear_graph_1",
                            ),
                ...
                gui.Column(
                    [
                        [
                            ...
                            gui.Button(
                                image_filename=self.icons_path
                                + "center_as_left_icon.png",
                                button_color=button_color,
                                tooltip="Center As Left",
                                key="center_as_left",
                            ),
                            gui.Button(
                                image_filename=self.icons_path + "ocr_icon.png",
                ...
                gui.Column(
                    [
                        [
                            ...
                            gui.Button(
                                image_filename=self.icons_path
                                + "calculate_diff_icon.png",
                                button_color=button_color,
                                tooltip="Calculate Difference",
                                key="calculate_diff",
                            ),
                            gui.Button(
                                image_filename=self.icons_path + "auto_center_icon.png",
                                button_color=button_color,
                                tooltip="Auto Center",
                                key="auto_center",
                            ),
                ...
                gui.Column(
                    [
                        [
                            gui.Text("Screenshot:"),
                            gui.Input(
                                default_text=self.screenshot_size[0],
                                size=(6, 1),
                                key=self.screenshot_width_key,
                                tooltip="Screenshot Width",
                            ),
                            gui.Input(
                                default_text=self.screenshot_size[1],
                                size=(6, 1),
                                key=self.screenshot_heigh_key,
                                tooltip="Screenshot Heigh",
                            ),
                            gui.Button(
                                button_text="Resize", tooltip="Resize Screenshot"
                            ),
                        ],
                        [
                            gui.Text("Comparison:"),
                            gui.Combo(
                                ["Heatmap", "Opacity", "Simple Diff"],
                                size=(8, 6),
                                enable_events=True,
                                key=self.comparison_mode_key,
                        ...
                        [
                            gui.Slider(
                                size=(int(self.screen_one_third_h // 9), 20),
                                range=(1, 100),
                                orientation="h",
                                key=self.comparison_strenght_key,
                                enable_events=True,
                                disable_number_display=True,
                                default_value=self.comparison_strenght,
                                tooltip="Comparison Strenght",
                            )
                        ],
                        [
                            gui.Multiline(
    ...
    def get_about_window(self):
        about_window = [
            ...
            [
                gui.Text(
                    "ver. " + self.detailist_app_version, font=self.small_font, p=(0, 0)
                )
            ],
            [
                gui.Text(
    ...
    def init_gui(self):
        ...
        self.window = gui.Window(
            ...
            location=(self.screen_quarter_w, 50),
        )
    ...
    def init_canvas(self, canvas):
        ...
        canvas.bind("<ButtonPress-1>", lambda event: self.canvas_click(event, canvas))
        canvas.bind(
            "<B1-Motion>", lambda event: canvas.scan_dragto(event.x, event.y, gain=1)
        )
        canvas.bind("<ButtonRelease-1>", lambda _: self.calculate_screenshots_diff())
        canvas.bind(
            "<Left>",
            lambda _: [
    ...
    def init_graphs(self):
        ...
        if len(self.screenshot_size) >= 2:
            ...
            self.graph_diff.draw_text(graph_diff_help_text, graph_help_location)
    ...
    def start(self):
        while True:
            ...
            if event in (
                "Compare Screenshots",
                gui.EVENT_SYSTEM_TRAY_ICON_DOUBLE_CLICKED,
            ):
                ...
                self.comparison_strenght = int(values[self.comparison_strenght_key])
                self.calculate_screenshots_diff()
    ...
    def resize_screenshots(self, screenshot_width_input, screenshot_heigh_input):
        if not screenshot_width_input or not screenshot_heigh_input:
            self.tray.show_message("Detailist", "Screenshot Width or Heigh is empty!")
            return
        ...
        try:
            ...
            if screenshot_width <= 0 or screenshot_heigh <= 0:
                self.tray.show_message("Detailist", "Screenshot size invalid value!")
                return
            ...
            self.tray.show_message("Detailist", "Screenshot size must be a number!")
    ...
    def get_pos_diff(self, sequence_1, sequence_2):
        sequence_matcher = SequenceMatcher(None, sequence_1, sequence_2, autojunk=False)
    ...
    def auto_center(self):
        ...
        self.graph_2.tk_canvas.xview_moveto(x_pos[0] - pos_diff_x / self.max_width)
        self.graph_2.tk_canvas.yview_moveto(y_pos[0] - pos_diff_y / self.max_heigh)
        self.is_center_in_progress = False
    ...
    def clear_graph(self, event):
        is_clear = gui.popup_yes_no("Are you sure you want to clear screenshot?")
        if is_clear != "Yes":
            return
    ...
    def clear_screenshot(self):
        if not self.is_screenshot_1 and not self.is_screenshot_2:
            self.tray.show_message("Detailist", "No screenshots to clear.")
            return
        ...
        self.tray.show_message("Detailist", "Screenshot cleared.")
    ...
    def create_screenshot(self):
        ...
        graph.erase()
        graph.draw_image(data=screenshot_data.getvalue(), location=(0, 0))
    ...
    def calculate_opacity_diff(self, image_1, image_2):
        translated_strenght = self.translation(self.comparison_strenght, 1, 100, 0, 1)
    ...
    def calculate_heatmap_diff(self, image_1, image_2):
        ...
        translated_strenght = self.translation(self.comparison_strenght, 1, 100, 0, 255)
    ...
    def calculate_screenshots_diff(self):
        ...
        self.graph_diff.draw_image(data=screenshot_data.getvalue(), location=(0, 0))
        self.is_diff_in_progress = False
